 ...
 * @param {number} topN - The number of top categories to include in the heatmap.
...
export function processSeriesData(data, dimensions, measures, topN) {
    ...
    if (!isNaN(topNFilter) && topNFilter > 0) {
        const sorted = Array.from(columnTotals.entries()).sort((a, b) => b[1] - a[1]).slice(0, topNFilter).map(entry => entry[0]);
